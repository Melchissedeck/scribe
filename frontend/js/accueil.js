import { getMeetings, deleteMeeting, getActions, getOverdueActions, getDashboardTrends, getDashboardSpeakingTime, ApiError } from './api.js';
...
loadTrendChart();
loadThemesChart();
loadActionsStatusChart();
loadSpeakingTimeChart();
  ...
  try {
    ...
    new Chart(canvas, {
      ...
      options: {
        ...
        scales: {
          x: { beginAtZero: true, ticks: { precision: 0, color: vizColor('--viz-muted'), font: { size: 11 } }, grid: { color: vizColor('--viz-grid') }, border: { display: false } },
          y: { ticks: { color: vizColor('--viz-ink-2'), font: { size: 11 } }, grid: { display: false }, border: { display: false } },
        },
      },
    });
  } catch (err) {
...
// ── Graphe des actions ouvertes / fermées ────────────────────────────────
async function loadActionsStatusChart() {
  const canvas = document.getElementById('chart-actions-status');
  const wrap = canvas.closest('.chart-canvas-wrap');
  const emptyEl = document.getElementById('actions-status-empty');
  const errorEl = document.getElementById('actions-status-error');
  ...
  try {
    ...
    const open = actions.filter((a) => a.status === 'todo' || a.status === 'in_progress').length;
    ...
    new Chart(canvas, {
      ...
      data: {
        ...
        datasets: [{
          data: [open, done],
          backgroundColor: [vizColor('--viz-series-2'), vizColor('--viz-series-3')],
          borderWidth: 0,
        }],
      },
      options: {
        ...
        plugins: {
          legend: {
            position: 'bottom',
            labels: { color: vizColor('--viz-ink-2'), font: { size: 11 }, boxWidth: 10, padding: 12 },
          },
        },
      },
    });
...
// ── Graphe du temps de parole ─────────────────────────────────────────────
async function loadSpeakingTimeChart() {
  const canvas = document.getElementById('chart-speaking-time');
  const wrap = canvas.closest('.chart-canvas-wrap');
  const emptyEl = document.getElementById('speaking-time-empty');
  const errorEl = document.getElementById('speaking-time-error');

  try {
    const { entries } = await getDashboardSpeakingTime();
    ...
    new Chart(canvas, {
      type: 'bar',
      data: {
        labels: top.map((e) => e.speaker),
        datasets: [{
          data: top.map((e) => e.percentage),
      ...
      options: {
        ...
        scales: {
          x: { beginAtZero: true, max: 100, ticks: { color: vizColor('--viz-muted'), font: { size: 11 }, callback: (v) => `${v}%` }, grid: { color: vizColor('--viz-grid') }, border: { display: false } },
          y: { ticks: { color: vizColor('--viz-ink-2'), font: { size: 11 } }, grid: { display: false }, border: { display: false } },
        },
      },
    });
...
function seriesPalette() {
  return ['--viz-series-1', '--viz-series-2', '--viz-series-3', '--viz-series-4', '--viz-series-5', '--viz-series-6']
    .map(vizColor);
}